...
export default async function handler(
    ...
    if (!token) {
        console.warn(`Profile fetch attempt for [${registrationType}] without auth token cookie.`);
        return res.status(401).json({ message: 'Authentication required.' });
    }

    const backendUrl = getBackendUrl(`/${registrationType}/profile`);
    ...
    try {
        ...
        console.log(`Backend [${registrationType}] profile response status: ${apiRes.status}`);
        ...
        console.error(`Error forwarding profile request for [${registrationType}] to backend:`, error);
