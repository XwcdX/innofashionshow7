  ...
  useEffect(() => {
    async function fetchData() {
      if (dataPetra.length === 0) {
        ...
        try {
          const resPetra = await fetch('/api/talkshow/petra', {
            credentials: 'include'
          })
          ...
          const modifiedDataPetra = rawDataPetra.map((item: any) => ({
            ...
            Pembayaran: item.status_pembayaran === false
              ? `<button class="btn-pay bg-blue-500 text-white px-4 py-2 rounded" data-href="talkshows" data-id="${item.id}">Validasi</button>`
      ...
      if (dataUmum.length === 0) {
        setLoadingUmum(true);
        try {
          const resUmum = await fetch('/api/talkshow/umum', {
            credentials: 'include'
          })
