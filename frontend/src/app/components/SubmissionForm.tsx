...
const determineUserType = (email: string | null | undefined): UserType => {
    ...
    const isInternal = localPart && domain === 'john.petra.ac.id' && /^[A-Za-z]\d{8}$/.test(localPart);
...
export function SubmissionForm({
    ...
    // --- API URLs ---
    const submitApiUrl = `/api/${registrationType}/submitCreation`;
    const uploadApiUrl = `/api/${registrationType}/uploadCreation`;
    const draftApiUrl = `api/${registrationType}/draftCreation`;
    const profileApiUrl = `api/${registrationType}/profileCreation`;
    ...
    useEffect(() => {
        if (status === 'unauthenticated') {
            const nextUrl = encodeURIComponent(`/contest`);
