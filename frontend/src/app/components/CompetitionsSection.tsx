      ...
      <div className="container mx-auto px-4 text-center">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 pt-20">
          {competitionsData.map((competition, index) => {
            const cardContent = (
              <div
                className="group relative p-8 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-xl shadow-lg transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:z-10 h-full flex flex-col justify-center" // Added h-full and flex for better alignment if descriptions vary
              >
                {/* Main Content */}
            ...
            if (competition.title === 'Competition' && competition.link) {
              return (
                <Link key={index} href={competition.link} passHref legacyBehavior={false} className="block h-full">
