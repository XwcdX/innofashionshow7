...
// frontend/src/app/components/InfiniteMenu.tsx

'use client';
import React, { useRef, useEffect } from 'react';
import { motion, useAnimation } from 'framer-motion';
import { Box, Button, Heading, Text, Image, Flex, Tooltip, Icon } from '@chakra-ui/react';
import { Session } from 'next-auth';
import { FaArrowRight } from 'react-icons/fa';

// Perbarui tipe Item untuk menyamakan dengan data dari API
type Item = {
  id: string;
  image: string;
  link?: string;
...
// Tambahkan props baru ke InfiniteMenu
...
const InfiniteMenu = ({ items, session, hasVoted, onVote }: InfiniteMenuProps) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const controls = useAnimation();

  useEffect(() => {
    const sequence = async () => {
      // Animasi yang sudah ada di file Anda
  ...
  // Duplikasi item untuk efek looping yang mulus
  const duplicatedItems = [...items, ...items];

  return (
    <Box position="absolute" top="50%" left="50%" transform="translate(-50%, -50%)" width="2000px" height="400px" overflow="hidden">
      <motion.div
        ref={containerRef}
        animate={controls}
        style={{ display: 'flex' }}
        ...
        {duplicatedItems.map((item, index) => (
          ...
          >
            ...
            <Box p={4}>
              ...
              <Text fontSize="sm" mt={2} noOfLines={3}>{item.description}</Text>
              <Text mt={2} fontWeight="bold">Votes: {item.voteCount}</Text>
            </Box>
            
            {/* >>> LOGIKA TOMBOL VOTE DITAMBAHKAN DI SINI <<< */}
            {session && !hasVoted && (
               <Tooltip label="Click to Vote" placement="right">
                <Flex
                  position="absolute"
                  top={0}
                  right={0}
                  ...
                  cursor="pointer"
                  onClick={() => onVote(item.id)} // Menjalankan fungsi onVote
                >
