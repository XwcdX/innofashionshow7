...
export const InputField: React.FC<InputFieldProps> = ({
    ...
    let specificInputClassName = "";
    if (field.type === 'number') {
        specificInputClassName += " [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";
        specificInputClassName += " [-moz-appearance:textfield]";
    }
    const commonProps = {
        id: field.id,
        name: field.id,
        ...
        className: `w-full border ${error ? 'border-red-500' : 'border-white/30'} 
                    px-3 py-2 rounded-md 
                    bg-white/10 backdrop-filter backdrop-blur-sm 
                    text-white placeholder-gray-300  // Ensure placeholder text is visible
                    focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 transition 
                    ${field.readOnly && field.type !== 'readonly' ? 'bg-gray-700/20 cursor-not-allowed text-gray-300' : ''} ${specificInputClassName}`,
        placeholder: field.placeholder,
    };

    const label = (
        <label htmlFor={field.id} className="block text-sm font-medium text-gray-200 mb-1 whitespace-pre-line">
            {field.label} {field.required && <span className="text-red-400">*</span>}
        </label>
    );

    const errorMsg = error && <p className="text-xs text-red-400 mt-1">{error}</p>;
    ...
    const confirmedUploadedFileDisplay = uploadedFilePath && (field.type === 'file' || field.type === 'file-drag-drop') && (
        <div className="text-xs text-green-400 [text-shadow:0_0_5px_rgba(0,255,0,0.4)] mt-1 flex items-center break-all">
                        ...
                        {field.options?.map(opt => (
                            <label key={opt.value} className="flex items-center space-x-2 text-sm text-gray-200">
                                <input
                                    ...
                                    required={field.required}
                                    disabled={disabled}
                                    className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-500 bg-transparent"
                                />
                                <span>{opt.label}</span>
                            </label>
                        ))}
                ...
                <div className="flex items-start my-0">
                    <div className="flex items-center h-5">
                        <input
                            ...
                            required={field.required}
                            disabled={disabled}
                            className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-500 rounded bg-white/20"
                        />
                    </div>
                    <div className="ml-3 text-sm">
                        <label htmlFor={field.id} className="font-medium text-gray-200">{field.label}</label>
                        {errorMsg}
                    </div>
                </div>
            );
            ...
            return (
                <div className='my-0'>
                    ...
                    <input
                        ...
                        className={`w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-500/30 file:text-indigo-100 hover:file:bg-indigo-600/40 transition cursor-pointer ${error ? 'border border-red-500 rounded-lg p-1' : 'border border-transparent'} ${field.readOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
                    {previewContent}
                ...
                <div className='my-0'>
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center justify-center text-center pointer-events-none w-full">
                            {dropzoneContent}
                            <p className={`text-sm max-w-full truncate px-2 ${currentFileNameForDisplay ? 'font-semibold' : 'text-gray-500'} ${isDragging ? 'text-indigo-700' : ''}`}>
                                {messageText}
                            </p>
